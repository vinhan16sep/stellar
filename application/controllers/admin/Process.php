...
class Process extends Admin_Controller {
    ...
    public function create() {
        ...
        try {
            ...
            $process_en_id = $this->process_en_model->get_last_id();
            $process_en_id = $process_en_id ? intval($process_en_id['id']) : 0;

            $forder = uniqid();
            $uploaddir = getcwd().'/assets/upload/process/'.$forder.'/';
            if (!file_exists($uploaddir)) {
                mkdir($uploaddir, 0777, true);
            }
            if($_FILES && $_FILES['img']['tmp_name']){
                foreach ($_FILES['img']['tmp_name'] as $key => $item){
                    move_uploaded_file($item, $uploaddir.$_FILES['img']['name'][$key]);
                    $process_image[] = array(
                        'process_id' => $process_id,
                        'process_en_id' => $process_en_id,
                        'content' => $forder.'/'.$_FILES['img']['name'][$key],
                        'is_delete' => 0
                    );
                }
            }
            $this->banners_model->insert_banner_image($process_image);
    ...
    public function edit($id) {
        ...
        $this->data['imgs'] = $this->banners_model->get_all_image_process($id);
        if ($this->input->post()) {
            $banner_data = array(
                'title' => $this->input->post('title'),
                'content' => $this->input->post('description')
            ...
            try {
                ...
                $process_en_id = $process_en['id'] ? $process_en['id'] : 0;
                if(!empty($process_en)){
    ...
    public function deleteimage($id) {
        ...
        $img = $this->banners_model->get_image_process_by_id($id);
        $this->banners_model->delete_image_process($id);

        redirect('admin/process/edit/'.intval($img[0]['process_id']), 'refresh');
    }
